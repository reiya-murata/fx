...
export function buildMonthlyPerformanceReport(state, now = new Date()) {
  ...
  const payload = {
    ...
    contextValidationCollectOnlyTradeCount: allAuto.filter((t) => t.contextValidationCollectOnly === true).length,
    byContextValidationMode: summarizeByFn(allAuto, (t) => t.contextValidationMode || t.validationMode || "UNKNOWN", init),
    byContextValidationCollectOnly: summarizeByFn(allAuto, (t) => t.contextValidationCollectOnly === true ? "collectOnly" : "normal", init),
